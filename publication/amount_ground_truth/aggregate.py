...
def main(test_path='/Volumes/secured/temp/publication/amount_ground_truth/test'):
    ...
    data2 = data.pivot_table(index=['amount', 'run', 'sample'], columns='label', values=metrics)
    ...
    data = data.groupby(['amount', 'label'])[metrics].mean().reset_index()

    data2 = data.pivot_table(index=['amount'], columns='label', values=metrics)
    data2.columns = ['{}_{}'.format(x, y) for x, y in
                     zip(data2.columns.get_level_values(1), data2.columns.get_level_values(0))]
    data2 = data2.reset_index()

    # save aggregated data (in wide format)
    data2.to_csv(os.path.join(test_path, 'averages_wide.csv'))
...
def read_run_from_csv(test_path, run, label):
    ...
    # add parameters
    df['amount'] = run   # run i uses i images for training set
    df['label'] = label
    df['run'] = run
