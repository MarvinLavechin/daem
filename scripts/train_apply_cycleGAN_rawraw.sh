...
if [ ! -d "$OUTPUT_DIR" ] || [ RANDOM_SEED_MODE ]; then
    eval $TRAIN_COMMAND
fi
...
if [ ! -d "$OUTPUT_DIR_RESULTS" ] || [ $RANDOM_SEED_MODE ]; then
    eval $TEST_COMMAND
fi
...
if [ ! -d "$OUTPUT_SEGMENTATION_TRANSLATED" ] || [ $RANDOM_SEED_MODE ]; then
